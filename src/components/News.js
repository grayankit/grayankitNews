...
export default class News extends Component {
  ...
  async componentDidMount() {
      let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=63f7ed9538ea40359414c6cb5fa0d6b2&pageSize=${this.props.pageSize}`;
  ...
  previousNews = async() => {
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=63f7ed9538ea40359414c6cb5fa0d6b2&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`;
    this.setState({loading:true})
    let data  = await fetch(url);
    let parsedData = await data.json();
    console.log(parsedData);
  ...
  render() {
    return (
      <div className="container my-3">
        <h2 className="text-center">India - Top Headlines</h2>
        ...
        <button type="button" disabled={this.state.page<=1} className="btn btn-outline-dark" onClick={this.previousNews}>&larr; Previous</button>
        <button type="button" disabled={this.state.page + 1 > Math.ceil(this.state.totalResults/this.props.pageSize)} className="btn btn-outline-dark" onClick={this.nextNews}>Next &rarr;</button>
        </div>
      </div>
    );
  }
